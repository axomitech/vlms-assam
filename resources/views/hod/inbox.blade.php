...
<div class="row">
    <div class="col-md-12">
        <div class="card">
            <div class="card-header"><strong>LETTER ACTIONS FORWARDED</strong></div>
            <div class="card-body">
               <br>
               <div class="row">
                <div class="col-md-12">
                   <div class="collapse" id="collapseExample">
                     ...
                     <div class="card card-body">
                        <!-- Additional content for collapse can be added here if needed -->
                     </div>
                   </div>
                 </div>
               </div>
               <div class="row" style="overflow: scroll; height: 300px;">
                  <div class="col-md-12">
                    <table class="table table-sm table-striped table-hover text-sm text-justify" id="letter-table">
                      <thead>
                          <tr class="text-sm">
                              <th>Sl No.</th>
                              <th>Action Forwarded</th>
                              <th>From</th>
                              <th>Respond</th>
                          </tr>
                      </thead>
                      <tbody>
                          ...
                          @foreach ($forwards as $value)
                            ...
                            <tr class="text-sm">
                                <td>{{ $i }}</td>
                                <td style="width: 35%;">{{ $value['action_description'] }}<br><b>Delivered: {{ $timeSpan }}</b></td>
                                <td>{{ $value['sender_name'] }}, {{ $value['department_name'] }}</td>
                                <td>
                                    <a class="file-btn" data-toggle="modal" data-target="#modal-lg" data-letter_path="{{ storageUrl($value['letter_path']) }}"><i class="fas fa-file-pdf"></i></a>
                                    &emsp;
                                    <a href="{{ route('responds', [encrypt($value['action_sent_id']), encrypt($value['act_dept_id']), encrypt($value['letter_id'])]) }}"><i class="fas fa-pen"></i></a>
                                </td>
                            </tr>
                            @php $i++; @endphp
                          @endforeach
...
$(document).on('click', '.note-link', function(e) {
    ...
    $.get("{{ route('action_notes') }}", { 'action': action }, function(j) {
        var tr = "";
