...
class ReportModel extends Model
{
    ...
    public static function get_diarized_report($category_id)
    {
        return DB::table('letters')
            ...
            ->leftjoin('user_departments','user_departments.id','=','letters.user_id')
            ->leftjoin('users','users.id','=','user_departments.user_id')
            ->leftjoin('senders','letters.id','=','senders.letter_id')
            ->select('crn','subject','letter_no','received_date','letter_path','letters.id AS letter_id','name','issue_date','ecr_no','sender_name','sender_designation','organization')
            ->where('letters.department_id', '=', session('role_dept'))
