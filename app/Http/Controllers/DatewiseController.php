...
class DatewiseController extends Controller
{
    ...
    public function datewise(Request $request)
    {
        ...
        <a href="javascript:void(0)" onclick="downloadMergedPDF()" class="btn btn-primary mt-3">📄 Download All PDFs (Merged)</a>

<script>
    function downloadMergedPDF() {
        const requestData = ' . json_encode($results) . ';
        fetch("' . route('pdf.merge.all') . '", {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "X-CSRF-TOKEN": "' . csrf_token() . '"
            },
            body: JSON.stringify({ results: requestData })
        })
        .then(response => {
        ...
        <table class="table table-hover table-striped table-sm table-responsive" id="letter-table">
        <thead>
            <tr>
                <th>#</th>
    ...
    public function mergeAll(Request $request)
    {
        ...
        foreach ($results as $item) {
            $path = storage_path('app/' . str_replace('public/', 'public/', $item['letter_path']));
            if (File::exists($path)) {
                try {
        ...
        $mergedPath = storage_path('app/public/merged/MergedLetters_' . time() . '.pdf');
