...
class LetterReportController extends Controller
{
    /**
     * Organize only issue_date letters into folders
     */
    public function organizeLettersIntoFolders()
    {
        // Filter only letters with issue_date
        ...
        foreach ($letters as $letter) {
            ...
            $year = $letter->issue_date ? Carbon::parse($letter->issue_date)->format('Y') : 'UnknownYear';
            $month = $letter->issue_date ? Carbon::parse($letter->issue_date)->format('F') : 'UnknownMonth';
    ...
    /**
     * View only issue_date letters grouped folder-wise
     */
    public function viewLettersFolderWise()
    {
        ...
        $letters = Letter::with(['category', 'subCategory'])
            ...
            ->orderBy('issue_date') // Use issue_date instead of received_date
            ->get()
            ->groupBy([
                'letter_category_id',
                ...
                function ($letter) {
                    return $letter->issue_date
                        ? Carbon::parse($letter->issue_date)->format('Y')
                        : 'UnknownYear';
                },
                function ($letter) {
                    return $letter->issue_date
                        ? Carbon::parse($letter->issue_date)->format('F')
                        : 'UnknownMonth';
                }
            ]);
        ...
        return view('files.letter-report', [
